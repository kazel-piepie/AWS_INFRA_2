...
# Standalone metadata collector: reads the Riot app secret and writes its own
# CloudWatch Logs. No MSK and no Bedrock access. The ai/rorr/${env} secret is
# already granted to every task role via the shared rorr_lol_task_secret
# attachment (rorr_secret_read policy).
